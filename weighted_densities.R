...
selected_steps <- c("100", "150", "200", "250", "300")
# No obstructions
...
d100 <- MASS::kde2d(x=s100$x, y=s100$y)# calculate density
...
dwt100 <- kde2d.weighted(x=s100$x, y=s100$y, w=s100$angle) # calculate density
...
dens <- c(mean(d100$z), mean(d150$z), mean(d200$z), mean(d250$z), mean(d300$z))
dens2 <- cbind(selected_steps, dens, rep("not weighted", length(dens)))
denswt <- c(mean(dwt100$z), mean(dwt150$z), mean(dwt200$z), mean(dwt250$z), mean(dwt300$z))
...
d100_c <- MASS::kde2d(x=s100_c$x, y=s100_c$y) # calculate non-weighted density
...
dwt100_c <- kde2d.weighted(x=s100_c$x, y=s100_c$y, w=s100_c$angle) # calculate weighted density
...
# Combine mean densities into one dataframe
dens_c <- c(mean(d100_c$z), mean(d150_c$z), mean(d200_c$z), mean(d250_c$z), mean(d300_c$z))
dens2_c <- cbind(selected_steps, dens_c, rep("not weighted", length(dens_c)))
denswt_c <- c(mean(dwt100_c$z), mean(dwt150_c$z), mean(dwt200_c$z), mean(dwt250_c$z), mean(dwt300_c$z))
...
d100_slope <- kde2d(x=s100_slope$x, y=s100_slope$y)
d150_slope <- kde2d(x=s150_slope$x, y=s150_slope$y) 
d200_slope <- kde2d(x=s200_slope$x, y=s200_slope$y) 
d250_slope <- kde2d(x=s250_slope$x, y=s250_slope$y) 
d300_slope <- kde2d(x=s300_slope$x, y=s300_slope$y) 
...
# Combine mean densities into one dataframe
dens_slope <- c(mean(d100_slope$z), mean(d150_slope$z), mean(d200_slope$z), mean(d250_slope$z), mean(d300_slope$z))
dens2_slope <- cbind(selected_steps, dens_slope, rep("not weighted", length(dens_slope)))
denswt_slope <- c(mean(dwt100_slope$z), mean(dwt150_slope$z), mean(dwt200_slope$z), mean(dwt250_slope$z), mean(dwt300_slope$z))
