...
2. It includes scripts for importing data from [LoggerPro][lp], which I use to track fish in video. These data can then be used for statistical analyses of their behaviour. Currently, I've separated these into three scripts: 
	...
	3. [`tracking_import_stepwise.R`][track3] is for videos with the largest number of fish, where all individuals were tracked for select frames of the video. The resulting dataframe is organised opposite to the other scripts and is therefore handled differently.
